 ...
 */
export async function sendManualOrderConfirmation({ clientEmail, clientName, productLabel }) {
  const transport = getTransport();
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #1a1a1a;">
        ...
        <ul style="font-size: 14px; color: #555; line-height: 1.9; padding-left: 18px;">
          <li>Your current CV (attached)</li>
          <li>Your LinkedIn profile URL</li>
          <li>The role or sector you're targeting</li>
 ...
 */
export async function notifyOwnerManualOrder({ clientName, clientEmail, productLabel, amount, sessionId }) {
  const transport = getTransport();
  await transport.sendMail({
    from: `"Asovix Orders" <${process.env.SMTP_USER}>`,
    to: process.env.OWNER_EMAIL,
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 500px; color: #1a1a1a;">
        ...
        <p style="font-size: 13px; color: #888; margin-top: 16px;">The client has been asked to reply with their CV, LinkedIn URL and target role. Deliver within 24 hours.</p>
      </div>
    `,
  });
}
