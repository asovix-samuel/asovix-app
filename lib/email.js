...
/**
 * Confirmation to the customer for a manually-fulfilled package (24h delivery)
 */
export async function sendManualOrderConfirmation({ clientEmail, clientName, productLabel, cvOnFile = false }) {
  const transport = getTransport();
  const first = (clientName || 'there').split(' ')[0];
  await transport.sendMail({
    from: `"Asovix" <${process.env.SMTP_USER}>`,
    to: clientEmail,
    subject: `Order confirmed — your ${productLabel} is being prepared`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #1a1a1a;">
        <div style="padding: 32px 0 16px;">
          <h1 style="font-size: 24px; color: #1B3A6B; margin: 0 0 4px;">Asovix<span style="font-style:italic">.</span></h1>
        ...
        <p style="font-size: 15px; line-height: 1.6;">Your <strong>${productLabel}</strong> order is confirmed. Because this package includes personal review, it's prepared by hand — you'll receive everything at this email address <strong>within 24 hours</strong>.</p>
        ${cvOnFile ? `<p style="font-size: 15px; line-height: 1.6;">We already have the CV you shared with your diagnosis, so there's no need to send it again.</p>` : ''}
        <p style="font-size: 15px; line-height: 1.6;">To get the sharpest result, reply to this email now with:</p>
        <ul style="font-size: 14px; color: #555; line-height: 1.9; padding-left: 18px;">
          ${cvOnFile ? '' : '<li>Your current CV (attached)</li>'}
          <li>Your LinkedIn profile URL</li>
          <li>The role or sector you're targeting${cvOnFile ? ' (if it has changed since your diagnosis)' : ''}</li>
          <li>A job posting you'd love to land (optional but powerful)</li>
        </ul>
        <p style="font-size: 15px; line-height: 1.6;">Talk soon,<br><strong>Samuel Adu</strong><br><span style="color: #1B3A6B;">Asovix</span></p>
      </div>
    `,
  });
}
...
/**
 * Alert Samuel: a manual package was purchased — action needed within 24h
 */
export async function notifyOwnerManualOrder({ clientName, clientEmail, productLabel, amount, sessionId, diagnosisId = '', cvOnFile = false }) {
  const transport = getTransport();
  await transport.sendMail({
    from: `"Asovix Orders" <${process.env.SMTP_USER}>`,
    to: process.env.OWNER_EMAIL,
    subject: `🔔 MANUAL ORDER — ${productLabel} — ${clientName || clientEmail} (€${amount})`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 500px; color: #1a1a1a;">
        <h2 style="color: #1B3A6B;">Manual fulfilment needed — 24h clock started</h2>
        <table style="font-size: 14px; line-height: 2; width: 100%;">
          <tr><td style="color: #888; width: 140px;">Package</td><td><strong>${productLabel}</strong></td></tr>
          <tr><td style="color: #888;">Client</td><td>${clientName || '-'} (${clientEmail})</td></tr>
          <tr><td style="color: #888;">Paid</td><td><strong style="color: #1B3A6B;">€${amount}</strong></td></tr>
          <tr><td style="color: #888;">Stripe session</td><td>${sessionId}</td></tr>
          ${diagnosisId ? `<tr><td style="color: #888;">From diagnosis</td><td><strong>${diagnosisId}</strong></td></tr>` : ''}
        </table>
        <p style="font-size: 13px; color: #888; margin-top: 16px;">${cvOnFile
          ? `Their CV is already with you — search your inbox for <strong>${diagnosisId}</strong> to find the diagnosis lead with it attached. They have been asked only for their LinkedIn URL and target role.`
          : diagnosisId
            ? `This came from diagnosis <strong>${diagnosisId}</strong> (search your inbox for it). They have been asked to reply with their CV, LinkedIn URL and target role.`
            : 'The client has been asked to reply with their CV, LinkedIn URL and target role.'} Deliver within 24 hours.</p>
      </div>
    `,
  });
}
