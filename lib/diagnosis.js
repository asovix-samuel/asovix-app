...
// At or above this price the result leads with a conversation rather than a
// card payment. A cold visitor is rarely ready to spend this much on a page
// they found minutes ago; lower-priced services go straight to checkout.
export const HIGH_TICKET_FROM = 145;

// Leave EMPTY until a candidate-facing booking event exists. The only Calendly
// event today is the B2B "Recruiter Optimization Call", which would confuse a
// job seeker. While empty, high-ticket results say "talk it through with
// Samuel" by email. Paste a booking URL here and they switch to "Book a call".
export const CANDIDATE_BOOKING_URL = '';

export const CONTACT_EMAIL = 'info@asovix.com';

// Why this recommendation, in plain terms. No outcome promises.
export const REC_REASON = {
  focused_cv: 'For the roles you are targeting, a clear single page matters more than depth. This is the lighter-touch version, priced to match.',
  cv_positioning: 'Your answers point to your CV as the main place your value is getting lost. This is our core method: we find the evidence first, then build the CV around one target role.',
  linkedin: 'Your answers suggest your CV may be doing its job but your profile is not. Employers look at LinkedIn before they meet you, and again after.',
  interview_prep: 'You are getting through on paper — the gap is in the room. This is a mock interview built around your target role, with structured feedback.',
  cv_linkedin: 'Your answers point to both your CV and your LinkedIn. Fixing one while the other tells a different story tends to cancel out the work.',
  cv_linkedin_letter: 'Your answers point to your written application as a whole — everything an employer reads before deciding whether to call you.',
  full_package: 'Your answers point to several gaps at once: how you come across on paper, online and in interviews. Fixing one while leaving the others tends to move the problem rather than solve it.',
};

// For high-ticket recommendations, an honest smaller place to start.
export const DOWNSELL = {
  full_package: 'cv_positioning',
  cv_linkedin_letter: 'cv_positioning',
};

export function nextStepFor(offerId) {
  const o = OFFERS[offerId];
  const high = !!o && o.price >= HIGH_TICKET_FROM;
  const down = high && DOWNSELL[offerId] ? OFFERS[DOWNSELL[offerId]] : null;
  return {
    tier: high ? 'high' : 'low',
    primary: high ? (CANDIDATE_BOOKING_URL ? 'book' : 'talk') : 'buy',
    bookingUrl: CANDIDATE_BOOKING_URL || null,
    downsell: down,
    reason: REC_REASON[offerId] || (o ? o.short : ''),
  };
}
