...
// Emitted as `recommendation_shown` (renamed from `service_recommended` on
// 2026-09-10 to match the funnel spec). `tier` is 'low' (direct checkout) or
// 'high' (conversation first).
export function trackServiceRecommended(productId, value, tier) {
  track('recommendation_shown', { product: productId, value, currency: 'EUR', tier: tier || '' });
}

// A CV was uploaded and read successfully. File type and size only — never content.
export function trackCvUploaded(fileType, sizeKb) {
  track('cv_uploaded', { file_type: fileType, size_kb: sizeKb });
}

// Any call to action on the diagnosis result: buy_now, buy_smaller,
// talk_to_samuel, book_call, see_all_services.
export function trackCtaClicked(cta, productId, tier) {
  track('cta_clicked', { cta, product: productId, tier: tier || '' });
}
