...
export async function generateDocxBuffers(claudeOutput) {
  ...
  // Each CV may start with a "TITLE: ..." line — extract it for email
  // descriptions and filenames, and strip it from the document body.
  ...
  const c1 = extractTitle(parts[1]);
  const c2 = extractTitle(parts[2]);
  const c3 = extractTitle(parts[3]);

  const [buf1, buf2, buf3] = await Promise.all([
    Packer.toBuffer(parseCVTextToDocx(c1.body, 'CV1')),
    Packer.toBuffer(parseCVTextToDocx(c2.body, 'CV2')),
    Packer.toBuffer(parseCVTextToDocx(c3.body, 'CV3')),
  ]);

  return {
    buf1, buf2, buf3,
    cv1Text: c1.body, cv2Text: c2.body, cv3Text: c3.body,
    titles: [c1.title, c2.title, c3.title],
  };
}
