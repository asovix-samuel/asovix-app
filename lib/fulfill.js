...
import { sendCVsToClient, notifyOwner, notifyOwnerFailure, sendManualOrderConfirmation, notifyOwnerManualOrder } from './email';
...
const PRODUCT_LABELS = {
  focused_cv: 'Focused CV',
  cv_positioning: 'CV Positioning',
  linkedin: 'LinkedIn Positioning',
  cover_letter: 'Cover Letter (add-on)',
  interview_prep: 'Interview Preparation',
  cv_linkedin: 'CV + LinkedIn',
  cv_linkedin_letter: 'CV + LinkedIn + Cover Letter',
  full_package: 'Full Career Positioning Package',
  revision_fee: 'Additional Revision',
};

export async function fulfillOrder(stripe, session) {
  const meta = session.metadata || {};
  const product = meta.product || 'cv_positioning';
  const clientName = meta.clientName || session.customer_details?.name || 'Client';
  const clientEmail = meta.clientEmail || session.customer_email || session.customer_details?.email;
  ...
  // ── Manually-fulfilled packages: confirm + alert Samuel, no generation ──
  if (product !== 'instant') {
    const label = PRODUCT_LABELS[product] || product;
    try {
      // Orders that started as a diagnosis carry its id, and whether the CV
      // already arrived with that lead — so nobody is asked for it twice.
      const diagnosisId = meta.diagnosisId || '';
      const cvOnFile = meta.cvOnFile === '1';
      await sendManualOrderConfirmation({ clientEmail, clientName, productLabel: label, cvOnFile });
      await notifyOwnerManualOrder({
        clientName, clientEmail, productLabel: label,
      ...
      if (piId) {
        await stripe.paymentIntents.update(piId, { metadata: { asovix_fulfilled: 'true' } });
      }
      return 'manual';
    } catch (err) {
      console.error('fulfill: manual order handling error:', err);
      try {
        await notifyOwnerFailure({
          clientName, clientEmail, sessionId: session.id,
          errorMessage: 'MANUAL ORDER (' + label + '): ' + (err.message || String(err)),
          brief: { target: label }, cvText: '', jd: '',
        });
